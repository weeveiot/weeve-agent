...
func DeployDataService(man manifest.Manifest, command string) error {
	...
	logger.Log.Info(deploymentID, fmt.Sprintf("%ving data service ...", command))

	dataServiceExists, err := DataServiceExist(man.ManifestUniqueID)
	if err != nil {
		logger.Log.Error(deploymentID, err)
	...
	if dataServiceExists {
		if command == CMDDeploy {
			logger.Log.Info(deploymentID, fmt.Sprintf("Data service %v, %v already exist!", man.ManifestUniqueID.ManifestName, man.ManifestUniqueID.VersionNumber))
			...
			if err != nil {
				logger.Log.Error(deploymentID, "Error while cleaning old data service -> ", err)
				manifest.SetStatus(man.ID, containerCount, man.ManifestUniqueID, model.EdgeAppError, false)
	...
	logger.Log.Info(deploymentID, "Iterating modules, pulling image into host if missing ...")
	...
	for _, module := range man.Modules {
		...
		if exists { // Image already exists, continue
			logger.Log.Info(deploymentID, fmt.Sprintf("Image %v, already exists on host", imgDetails.ImageName))
		} else { // Pull this image
			logger.Log.Info(deploymentID, fmt.Sprintf("Image %v, does not exist on host", imgDetails.ImageName))
			logger.Log.Info(deploymentID, "Pulling ", imgDetails.ImageName)
			err = docker.PullImage(imgDetails)
			if err != nil {
				msg := "Unable to pull image/s, " + err.Error()
				logger.Log.Error(deploymentID, msg)
				manifest.SetStatus(man.ID, containerCount, man.ManifestUniqueID, model.EdgeAppError, false)
	...
	logger.Log.Info(deploymentID, "Creating network ...")
	...
	logger.Log.Info(deploymentID, "Created network >> ", networkName)

	//******** STEP 4 - Create, Start, attach all containers *************//
	logger.Log.Info(deploymentID, "Starting all containers ...")
	containerConfigs := man.Modules

	if len(containerConfigs) == 0 {
		logger.Log.Error(deploymentID, "No valid contianers in Manifest")
		manifest.SetStatus(man.ID, containerCount, man.ManifestUniqueID, model.EdgeAppError, false)
		logger.Log.Info(deploymentID, "Initiating rollback ...")
		UndeployDataService(man.ManifestUniqueID, CMDRemove)
		return errors.New("no valid contianers in manifest")
	}

	for _, containerConfig := range containerConfigs {
		logger.Log.Info(deploymentID, "Creating ", containerConfig.ContainerName, " from ", containerConfig.ImageName, ":", containerConfig.ImageTag)
		containerID, err := docker.CreateAndStartContainer(containerConfig)
		if err != nil {
			logger.Log.Error(deploymentID, "Failed to create and start container ", containerConfig.ContainerName)
			manifest.SetStatus(man.ID, containerCount, man.ManifestUniqueID, model.EdgeAppError, false)
			logger.Log.Info(deploymentID, "Initiating rollback ...")
			UndeployDataService(man.ManifestUniqueID, CMDRemove)
			return err
		}
		logger.Log.Info(deploymentID, "Successfully created container ", containerID, " with args: ", containerConfig.EntryPointArgs)
		logger.Log.Info(deploymentID, "Started!")
	}
...
func StopDataService(manifestUniqueID model.ManifestUniqueID) error {
	...
	logger.Log.Infoln("Stopping data service:", manifestUniqueID.ManifestName, manifestUniqueID.VersionNumber)

	containers, err := docker.ReadDataServiceContainers(manifestUniqueID)
	if err != nil {
		logger.Log.Error("Failed to read data service containers.")
	...
	for _, container := range containers {
		if container.State == stateRunning {
			logger.Log.Info("Stopping container:", strings.Join(container.Names[:], ","))
			err := docker.StopContainer(container.ID)
			if err != nil {
				logger.Log.Error("Could not stop a container")
				manifest.SetStatus("", 0, manifestUniqueID, model.EdgeAppError, false)
				return err

			}
			logger.Log.Info(strings.Join(container.Names[:], ","), ": ", container.Status, " --> exited")
		} else {
			logger.Log.Debugln("Container", container.ID, "is", container.State, "and", container.Status)
		}
	}
...
func StartDataService(manifestUniqueID model.ManifestUniqueID) error {
	logger.Log.Infoln("Starting data service:", manifestUniqueID.ManifestName, manifestUniqueID.VersionNumber)
	...
	for _, container := range containers {
		if container.State == stateExited || container.State == stateCreated || container.State == statePaused {
			logger.Log.Info("Starting container:", strings.Join(container.Names[:], ","))
			err := docker.StartContainer(container.ID)
			if err != nil {
				logger.Log.Errorln("Could not start a container", err)
				manifest.SetStatus("", 0, manifestUniqueID, model.EdgeAppError, false)
				return err
			}
			logger.Log.Info(strings.Join(container.Names[:], ","), ": ", container.State, "--> running")
		}
	}
...
func UndeployDataService(manifestUniqueID model.ManifestUniqueID, command string) error {
	logger.Log.Infoln("Undeploying data service:", manifestUniqueID.ManifestName, manifestUniqueID.VersionNumber)
	...
	if !dataServiceExists {
		logger.Log.Errorln(deploymentID, "Data service", manifestUniqueID.ManifestName, manifestUniqueID.VersionNumber, "does not exist")
		manifest.SetStatus("", 0, manifestUniqueID, model.EdgeAppError, false)
		return nil
	}
	...
	if err != nil {
		logger.Log.Error(deploymentID, "Failed to read data service containers.")
		manifest.SetStatus("", 0, manifestUniqueID, model.EdgeAppError, false)
		return err
	}
	...
	if command == CMDRemove {
		...
		if err != nil {
			logger.Log.Error(deploymentID, "Failed to read all containers.")
			manifest.SetStatus("", 0, manifestUniqueID, model.EdgeAppError, false)
			return err
		}
		...
		for imageID := range numContainersPerImage {
			...
			if numContainersPerImage[imageID] == 0 {
				logger.Log.Info(deploymentID, "Remove Image - ", imageID)
	...
	logger.Log.Info(deploymentID, "Pruning networks ...")
...
func UndeployAll() error {
	...
	logger.Log.Info(knownManifests)
	...
	if len(knownManifests) > 0 {
		...
		logger.Log.Info("All the edge applications have been undeployed")
	} else {
		logger.Log.Info("No edge application to undeploy")
	}
	return nil
}
